...
const createComplaint = async (req, res) => {
  try {
    ...
    const { title, description, location, category, department: departmentName, severity } = req.body;
    ...
    // Auto-assign department based on user override or Nagarik360 mapping
    const departmentOverride = departmentName?.trim() || '';
    const nagrikDeptName = aiResult.nagrik_department || '';
    let department = null;

    if (departmentOverride) {
      department = await Department.findOne({
        name: { $regex: new RegExp(`^${departmentOverride}$`, 'i') },
        isActive: true,
      });
    }
    ...
    const complaint = await Complaint.create({
      ...
      media,
      severity: severity || aiResult.severity || 'medium',
      aiClassification: {
